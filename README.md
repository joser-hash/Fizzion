...
- **Bonus portals** start appearing ~45 s in: a smaller ring wrapped in gold ("BONUS") opens for 10 s somewhere clear of the main portal, every 40-60 s. Delivering into it pays **double Sparks**, counts toward your chain, and restores stability like a normal delivery — but missing it costs nothing, and it never affects relocation, boost pacing, or the FTUE ramp. Pure opportunity. A coach line explains it while one is on screen, reappearing with every spawn until you bank your first bonus delivery.
- **In-run boosts (roguelite picks)**: at the 3rd delivery — and with portal relocations after that (spaced at least 4 deliveries apart; a jump landing sooner defers its cards to the next one) — the run freezes and offers 3 rarity-weighted boost cards (common/rare/epic). Picks are unique per run and stack into a build: chain-window and request-timer extensions, thief bounties, harder swipes, overload debris that flies back on its own (only up to a comfortable working mass, so it never re-pops you or crowds out your next moves), a chain-break shield, a 4th pip slot, overload-into-the-portal delivery, and chain halving instead of resets. One rewarded-ad reroll per offer. The cards arm only after a short announce window, so a swipe in progress when the run freezes can't misfire into a pick.
